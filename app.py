...
def _build_demo():
    ...
    with gr.Blocks(title="Parallel Life Engine") as demo:
        ...
        with gr.Column(elem_id="ple-shell"):
            ...
            with gr.Row(elem_classes=["ple-section"], equal_height=True):
                ...
                with gr.Column(scale=4, elem_classes=["ple-card", "ple-runtime"], elem_id="runtime-card"):
                    gr.HTML(
                        ...
                        <p class="ple-blurb">This panel tracks the live run: fork recovery, memoir, timeline, portraits, then export.</p>
                        """
                    )
            ...
            with gr.Row(elem_classes=["ple-section"], equal_height=True):
                with gr.Column(scale=5, elem_classes=["ple-card", "ple-output"], elem_id="timeline-card"):
                    gr.HTML(
                        """
                ...
                with gr.Column(scale=5, elem_classes=["ple-card", "ple-output"], elem_id="shelf-card"):
                    gr.HTML(
                        """
